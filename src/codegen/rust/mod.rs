...
pub fn emit(world: &World, global: &GlobalOptions, out: &mut Outfile) -> Result<()> {
    if let Some(l) = &global.rust_types_as_bytes {
        for v in world.definitions.values() {
            if l.iter().find(|x| v.ident.as_str() == x.as_str()).is_some() {
                if !matches!(v.kind, TypeKind::DynamicArray(_)) {
                    return Err(anyhow::anyhow!("type {} is not a dynamic array", v.ident))
                        .context("replacing arrays with Bytes");
                }
            }
        }
    }
...
fn emit_preamble(global: &GlobalOptions, out: &mut impl Sink) -> Result<()> {
